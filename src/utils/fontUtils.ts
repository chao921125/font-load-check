 ...
 */
export function addFont(fontName: string, url: string, options?: FontFaceDescriptors, checkerOptions?: FontCheckerOptions): boolean {
  const checker = getGlobalFontChecker(checkerOptions);
 ...
 * @returns 布尔值，表示是否添加成功
 */
export function addFontFace(font: FontFace, options?: FontCheckerOptions): boolean {
  const checker = getGlobalFontChecker(options);
 ...
 */
export function deleteFont(font: FontFace | string, options?: FontCheckerOptions): boolean {
  const checker = getGlobalFontChecker(options);
